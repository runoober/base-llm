...
如图 6-34 所示，我们在 DeepSeek 的网页端测试一下这个输入。可以看到在这个场景下，模型不只是机械地弹出一个分类标签，而是输出了一句完整的自然语言“评论表达的情感：负面。”，但它准确地落在了 `负面` 这个情感情绪词汇上。之所以能够做到这一点，不是因为模型在预训练时见过完全一样的“判断该电影评论”的任务，而是得益于模型在海量语料上预训练所建立的**共享语义空间**。在预训练阶段，模型学习到了“电影”、“剧情”、“拖沓”等词汇的表征，以及这些词汇与“负面”情感概念在潜空间中的深层关联。而当我们下达自然语言指令时，零样本学习本质上是依靠模型经过对齐微调后形成的强大指令遵循能力 [^1]。通过解析指令的意向，自回归生成机制会将当前的新任务映射到其已构建的底层语义概念上。这种基于先验知识的语义映射会直接影响模型预测下一个 token 时的**概率分布**。模型不仅能依次采样输出结构完整的句子，而且当生成到表达核心态度的那一步时，代表“负面”的 token 会因上下文的语义引导而获得极高的生成概率，进而在不更新任何权重的条件下顺畅地完成任务。
...
少样本学习之所以有效，是因为它在上下文窗口中构建了一个临时的任务概率分布。受上下文连续多个一致性示例的驱动，模型的注意力机制能够极其敏锐地提炼并模仿这种有悖常理的映射关系，并将其完美泛化到当前的目标输入上。这证明了大语言模型不仅能回忆静态知识，还能实时地在复杂上下文中进行动态的全新模式匹配与学习。
...
<div align="center">
  <img src="images/6_4_5.png" width="90%" alt="标准提示词与思维链（CoT）的效果对比" />
...
如图 6-40 所示，模型虽然列出了“一步步”的分析框架，但在关键逻辑上依然出现了严重的幻觉（例如在第 3 点中假设人走过去车就在那里），最后还是给出了“走路去”的错误结论。这是一种只具其形的“伪推理”现象。
...
这一次模型如图 6-41 成功发现了盲点。它准确推理出“如果走路去，车还在家里，所以没法洗车”，得出“应该开车去”的正解。
...
>
> 这暴露出基础大语言模型在多语言推理能力表征空间中存在不均衡分布的不良现象。在模型的预训练阶段，包含严密逻辑推导的高质量数据（如数学解答、编程逻辑、学术论文等），它英文语料的积累时间往往更长、在主流开源数据集中的占比结构也更为庞大。也就导致大模型在早期训练时，更容易在英文语义空间内构建起相对致密且稳健的推理网络。同时，诸如“Let's think step by step”这类极高频的经典触发词，广泛存在于各种微调和评测数据集中，产生了强烈的“锚点”效应。它就像一把确切的特定钥匙，能够精准激活模型深层权重中最强大的一批推理回路。而且，如图 6-41 所示，在使用英文触发词后，模型不仅算对了答案，后续的整个推导过程也全都变为了英文。这种表现也直观印证了在 DeepSeek 模型内部这片最具连贯逻辑、最为强大的表征空间，与英语这一自然语言是深度绑定的。相比之下，中文的直译版本往往只能唤醒常规的对话生成区域，模型仅学到了使用“1、2、3”进行分点作答的表面排版格式，没有真正调动起深层的认知约束来审视逻辑冲突。本质上，通过显式要求生成中间推导步骤并使用能精准唤醒对应回路的特征词，模型其实在使用前文刚刚生成的词汇空间作为延伸的“草稿纸”。这些“草稿纸”上的中间结果将继续引导下一步的文本生成，廉价且有效地解决了模型因“直觉短路”而产生的局部逻辑盲区。

思维链技术的生效归根结底仍然高度依赖于底层训练数据。常规模型只有在预训练阶段见过海量的推理论证逻辑（如数学推导过程、代码演进逻辑等），才具备在推理阶段被特定提示词激活的先决条件。随着业界对大模型复杂推理能力的追求，最新一代的专用推理模型（如 OpenAI o1、DeepSeek-R1 等）已经完全跨越了单纯的提示词范畴。这类模型在预训练之后，通过大规模的强化学习以及引入海量包含详细推导过程的数据进行高强度监督微调，将“思维链”的精细推导能力直接内化到了自身的模型权重之中。当用户提出复杂或充满陷阱的问题时，这类模型会自动在后台自主展开深度的探索、试错与验证，彻底模糊了前端提示词技巧与模型底层基座能力之间的传统边界。
...
前文提到的诸如 OpenAI o1 或 DeepSeek-R1 等**专用推理模型**，标志着大模型**底层能力维度的范式进化**。为了实现“将探索、试错与自省的推导能力彻底内化于基座权重”，这些新型模型在训练阶段经历了范式的转变。以 DeepSeek-R1 为例，实现这一跨越的核心机制涉及两个主要阶段。第一阶段是**强化学习主导**，在基础预训练之后，模型不再单纯依赖传统的人类偏好对齐，还会通过大规模的强化学习提升逻辑推导能力。在数学求解或代码生成等具有明确结果反馈的场景中，系统基于规则的奖励机制引导模型自我探索。模型在试错中逐渐学习到通过延长推导链条以获取更高奖励的策略，在参数层面形成探索、推导、纠错的系统性思维模式。第二阶段则是**海量推理数据的监督微调**，在强化学习初步建立起推理能力后，研究团队会收集数十万乃至上百万条包含完整推理轨迹的数据（涉及详细的试错、条件剥离、公式代入等结构化步骤），作为高质量样本对模型进行进一步的监督微调。这一阶段的目的是固化并规范模型长序列后验推理的输出特征与格式，如图 6-44 展示了这类用于微调的推理数据集示例。
...
结合上述机制，专用推理模型在接收到复杂用户请求时，会自动在后台启动一个推导过程（耗时会持续数秒到数分钟不等），模型将在内部执行类似树状展开、剪枝与回溯的探索操作，直至得出收敛的结论后才会输出最终结果。如图 6-45，我们使用 DeepSeek 的推理模型对“洗车问题”进行测试，可以看到它的思考内核经历了“评估走路（距离近）、发现盲点（车没带过去）、尝试发散解决（让人把车开过去但麻烦）、最终收敛回溯（必须开车去）”的完整自省链条。
...
不过，在带来更好推理效果的同时，这类模型也伴随显著的代价，首当其冲的便是**推理期算力开销的成倍增加**以及**首字响应延迟的延长**。所以，对于日常问答、常规翻译或直接的信息检索等无需长逻辑链条干预的基础级任务，调用满血的推理模型会造成高强度的资源浪费。在实际的系统工程架构设计中，通常需要引入路由网关模块来实现不同复杂度的任务级动态分发，比如将常规类任务调度给基础生成模型，而将复杂多步规划任务路由给推理模型，以此平衡生成质量与计算成本。
