...
前文介绍的**基于词典构建 DAG 并利用动态规划寻找最大概率路径**的方法，在 `jieba` 中对应的就是其核心的**精确模式**。该模式力图将句子尽可能精确地切开，适合绝大多数文本分析场景。下面我们通过代码来看看这种分词方式的效果。
...
上面的例子之所以能切得比较理想，是因为“清华大学”“通知书”等词在词典中已有较稳定的统计信息。反过来，精确模式的核心仍然是“基于词典找最优路径”。一旦遇到词典里没有收录的新词（例如“奔波儿灞”），就容易被拆成更小的片段，出现不符合预期的切分。这时，**基于词典的分词方法**的优势就体现出来了。我们可以进行**人工干预**，通过**自定义词典**把新词加入词表，“教会”`jieba`认识它。接下来创建一个自定义字典`user_dict.txt`：
